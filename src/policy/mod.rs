...
impl PolicyEngine {
    ...
    pub fn evaluate(&self, tool_call: &ToolCall) -> PolicyDecision {
        // Warn-tier deny rules do not short-circuit: hold them and keep looking so
        // higher-severity BLOCK rules from later sections cannot be shadowed by a
        // harmless path/tool mention. For example, a command that mines a workflow
        // path still must be blocked by `curl | sh`, and content written to a
        // warn-tier path still must be blocked when it contains a block-tier secret.
        // BLOCK (or explicit allow-action) rules still return immediately.
...
"#;
    ...
    fn block_secret_overrides_warn_tier_path() {
        // A warn-tier path must not downgrade a block-tier secret in the same
        // tool call. The path warning is held while deny.secrets is evaluated,
        // and a block-tier secret wins.
        let engine = PolicyEngine::from_config(PolicyConfig::new(
            PolicySettings { mode: "enforce".into(), on_failure: "closed".into(), default: "warn".into() },
            vec![DenyPathRule { pattern: "**/.env".into(), action: "warn".into(), reason: "env file".into() }],
            vec![],
        ...
        let decision = engine.evaluate(&call);
        assert_eq!(decision.action, Action::Block, "block-tier secret must override the warn-tier path match");
